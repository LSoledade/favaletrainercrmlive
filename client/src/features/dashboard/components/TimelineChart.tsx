...
interface SessionData {
  ...
  interface TimelineChartEntry {
    color: string; // Recharts provides 'color' in payload object, typically same as 'stroke' or 'fill'
    name: string;
    value: number;
    // payload might contain the original data point if needed: payload: LeadTimeData;
  }
  interface CustomTooltipProps {
    active?: boolean;
    payload?: TimelineChartEntry[];
    label?: string | number;
  }
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white dark:bg-slate-800 shadow-md dark:shadow-glow-xs rounded p-2 text-xs sm:text-sm border border-gray-100 dark:border-primary/30">
          ...
          {payload.map((entry: TimelineChartEntry, index: number) => ( // Updated entry type
