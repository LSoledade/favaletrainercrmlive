...
import { useLeads } from "@/features/leads/hooks/useLeads"; // Added useLeads hook
import { ptBR } from "date-fns/locale";
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/feedback/tooltip"; // Updated path
...
export default function RecentActivity({ className = "" }: RecentActivityProps) {
  const { data: leads, isLoading, error } = useLeads(); // Use the new hook

  // Handle error state if needed
  if (error) {
    // console.error("Error fetching leads for RecentActivity:", error);
    // return <Card className={`p-3 sm:p-5 ${className}`}>Error loading activities.</Card>; // Or some other error UI
  }

  const generateActivities = (leadsData: Lead[]): Activity[] => { // Renamed variable to avoid conflict
    ...
    // Static campaign activity for demonstration
    if (activities.length < 5) { // Add only if there's space
        activities.splice(Math.min(2, activities.length), 0, {
        id: "activity-campaign",
        type: "campaign",
        title: "Nova campanha Verão 2023 iniciada",
        description: "",
        time: "Há 2 horas", // This time should be dynamic or handled differently
        icon: "campaign",
  ...
  const displayActivities = leads ? generateActivities(leads) : []; // Renamed variable
