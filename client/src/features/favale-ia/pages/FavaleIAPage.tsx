...
import { Card, CardContent, CardHeader, CardTitle } from '@/components/data-display/Card'; // Updated
import { Avatar, AvatarFallback } from '@/components/data-display/avatar'; // Updated
...
import { MarkdownRenderer } from '@/components/data-display/MarkdownRenderer'; // Updated
...
interface RawMessage {
  ...
  content: string;
  role: 'user' | 'assistant';
  timestamp: string; // Before parsing
}
...
interface RawConversation {
  ...
  title: string;
  messages: RawMessage[];
  createdAt: string; // Before parsing
}
...
export function FavaleIAPage() {
  ...
  useEffect(() => {
    ...
    if (savedConversations) {
      try {
        const parsed: RawConversation[] = JSON.parse(savedConversations); // Assume parsed is RawConversation[]
        const conversationsWithDates: Conversation[] = parsed.map((conv: RawConversation) => ({ // Updated conv type
          ...conv,
          createdAt: new Date(conv.createdAt),
          messages: conv.messages.map((msg: RawMessage) => ({ // Updated msg type
            ...msg,
