...
export default function RecentActivity({ className = "" }: RecentActivityProps) {
  ...
  return (
    <Card variant="glow" className={`flex flex-col h-full p-3 sm:p-5 ${className}`}>
      <div className="flex justify-between items-center mb-3 sm:mb-4">
        <h3 className="font-heading text-base sm:text-lg font-medium dark:text-white dark:glow-text-subtle">Atividades Recentes</h3>
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <button className="text-gray-400 hover:text-secondary transition-all duration-200 dark:text-gray-300 dark:hover:text-pink-400 hover:scale-110">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-200 dark:scrollbar-thumb-slate-700 scrollbar-track-transparent">
        {isLoading ? (
          <div className="flex justify-center py-4">
            <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-primary dark:border-pink-400 dark:shadow-glow-xs"></div>
          </div>
        ) : activities.length > 0 ? (
          <div className="space-y-3 sm:space-y-4">
            {activities.map((activity) => (
              <div key={activity.id} className="flex items-start group hover:bg-gray-50 dark:hover:bg-slate-700/50 p-2 rounded-lg transition-all duration-200 hover-lift-sm">
                <div className={`${activity.iconBgColor} rounded-full w-6 h-6 sm:w-8 sm:h-8 flex items-center justify-center mr-2 sm:mr-3 flex-shrink-0 dark:shadow-glow-xs transition-all duration-200 group-hover:scale-110`}>
