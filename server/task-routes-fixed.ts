...
async function userHasAccessToTask(userId: number, taskId: number): Promise<boolean> {
  try {
    ...
    const watchers = await taskStorage.getTaskWatchers(taskId);
...
export function registerTaskRoutes(app: Express) {
  ...
  app.get("/api/tasks", async (req: Request, res: Response) => {
    try {
      ...
      if (isUserAdmin) {
        ...
        const assignedTasks = await taskStorage.getTasksByUserId(req.user.id);
  ...
  app.post("/api/tasks", isAdmin, async (req: Request, res: Response) => {
    try {
      ...
      // Criar a tarefa
      const task = await taskStorage.createTask(taskData);
      ...
      if (task.createdById !== task.assigneeId) {
        await taskStorage.addTaskWatcher({
          taskId: task.id,
      ...
      if (taskData.checklistItems && Array.isArray(taskData.checklistItems)) {
        for (let i = 0; i < taskData.checklistItems.length; i++) {
          ...
          await taskStorage.createTaskChecklistItem({
            taskId: task.id,
      ...
      await taskStorage.createTaskHistory({
        taskId: task.id,
        userId: req.user!.id,
        action: "created",
        details: { 
  ...
  app.get("/api/tasks/:id", async (req: Request, res: Response) => {
    try {
      ...
      const taskWithDetails = await taskStorage.getTaskByIdWithDetails(taskId);
  ...
  app.post("/api/tasks/:id/comments", async (req: Request, res: Response) => {
    try {
      ...
      // Criar o comentário
      const comment = await taskStorage.createTaskComment({
        taskId,
        userId: req.user.id,
  ...
  app.get("/api/tasks/:id/comments", async (req: Request, res: Response) => {
    try {
      ...
      // Obter comentários
      const comments = await taskStorage.getTaskComments(taskId, page, limit);
  ...
  app.post("/api/tasks/:id/checklist", isAdminOrAssignee, async (req: Request, res: Response) => {
    try {
      ...
      const existingItems = await taskStorage.getTaskChecklistItems(taskId);
      const nextPosition = existingItems.length > 0 
        ? Math.max(...existingItems.map(item => item.position)) + 1 
        : 0;
      
      // Criar o item de checklist
      const checklistItem = await taskStorage.createTaskChecklistItem({
        taskId,
        content: req.body.content,
  ...
  app.patch("/api/tasks/:taskId/checklist/:itemId", isAdminOrAssignee, async (req: Request, res: Response) => {
    try {
      ...
      // Atualizar o item
      const updatedItem = await taskStorage.updateTaskChecklistItem(itemId, updates);
  ...
  app.delete("/api/tasks/:taskId/checklist/:itemId", isAdminOrAssignee, async (req: Request, res: Response) => {
    try {
      ...
      // Excluir o item
      const deleted = await taskStorage.deleteTaskChecklistItem(itemId);
  ...
  app.post("/api/tasks/:id/watchers", isAdminOrAssignee, async (req: Request, res: Response) => {
    try {
      ...
      const watcher = await taskStorage.addTaskWatcher({
        taskId,
        userId
      });
  ...
  app.delete("/api/tasks/:taskId/watchers/:userId", isAdminOrAssignee, async (req: Request, res: Response) => {
    try {
      ...
      // Remover observador
      const removed = await taskStorage.removeTaskWatcher(taskId, userId);
  ...
  app.get("/api/tasks/:id/history", async (req: Request, res: Response) => {
    try {
      ...
      // Obter histórico
      const history = await taskStorage.getTaskHistoryByTaskId(taskId);
